...
export const fcc = (app) => {
	...
	app.route('/_api/routes/translate.js').get((req, res, next) => {
		console.log('requested');
		fs.readFile(__dirname + '/routes/translate.js', (err, data) => {
			if (err) return next(err);
			return res.type('txt').send(data.toString());
		});
	});

	app.route('/_api/controllers/translate.js').get((req, res, next) => {
		console.log('requested');
		fs.readFile(__dirname + '/controllers/translate.js', (err, data) => {
			if (err) return next(err);
			return res.type('txt').send(data.toString());
		});
	});
